...
// Fonts
surface.CreateFont("TraitorState", {font = "Open Sans",
                                    size = 28,
                                    weight = 1000})

surface.CreateFont("TimeLeft", {font = "Open Sans",
                                    size = 24,
                                    weight = 800})

surface.CreateFont("HealthAmmo", {font = "Open Sans",
                                    size = 24,
                                    weight = 750})
...
local function PaintBar(x, y, w, h, colors, value)
   ...
   if width > 0 then
      ...
      draw.RoundedBox(0, x, y, width, h, colors.fill)
   end
end
...
local function DrawBg(x, y, width, height, client)
   ...
   RoundedBoxBlur(x, y, width, height, 2, 5)
   ...
   draw.RoundedBox(0, x, y, tw, th, col)
end
...
local function SpecHUDPaint(client)
   ...
   /*draw.RoundedBox(0, x, round_y, width, height, Color(0, 0, 0, 150))
   draw.RoundedBox(0, x, round_y, time_x - x, height, Color(0, 0, 0, 150))*/
   RoundedBoxBlur(x, round_y, width, height, 2, 5)
   draw.RoundedBox(0, x, round_y, width, height, Color(0, 0, 0, 150))
   draw.RoundedBox(0, x, round_y, width * .63, height, Color(0, 0, 0, 150))

   local text = L[ roundstate_string[GAMEMODE.round_state] ]
   draw.SimpleText(text, "TraitorState", x + margin, round_y, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP)

   -- Draw round/prep/post time remaining
   local text = util.SimpleTime(math.max(0, GetGlobalFloat("ttt_round_end", 0) - CurTime()), "%02i:%02i")
   draw.SimpleText(text, "TimeLeft", width * .75, time_y, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP)

   local tgt = client:GetObserverTarget()
   if IsValid(tgt) and tgt:IsPlayer() then
      --ShadowedText(tgt:Nick(), "TimeLeft", ScrW() / 2, margin, COLOR_WHITE, TEXT_ALIGN_CENTER)
      draw.SimpleText(tgt:Nick(), "TimeLeft", ScrW()  /2, margin, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
...
local function InfoPaint(client)
   ...
   smooth = Lerp(0.9 * FrameTime(), smooth, (width - 10) * (health / client:GetMaxHealth()))

   --PaintBar(margin + 5, health_y, smooth - 10, bar_height, health_colors, health/100)
   draw.RoundedBox(0, margin + 5, health_y, smooth, bar_height, Color(200, 50, 50, 250))

   --ShadowedText(tostring(health), "HealthAmmo", bar_width, health_y, COLOR_WHITE, TEXT_ALIGN_RIGHT, TEXT_ALIGN_RIGHT)
   draw.SimpleText(tostring(health), "HealthAmmo", margin * 2, health_y, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_RIGHT)
   ...
   if client:GetActiveWeapon().Primary then
      ...
      if ammo_clip != -1 then
         ...
         draw.SimpleText(text, "HealthAmmo", bar_width, ammo_y, color_white, TEXT_ALIGN_RIGHT, TEXT_ALIGN_RIGHT)
      end
   end
